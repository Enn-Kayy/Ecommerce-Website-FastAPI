...
# 4️⃣ List all cart items for a user
...
def create_order_from_cart(
    ...
    for cart_item in cart.cart_items:
        ...
        if product.stock_qty is None:
            raise ValueError(
                f"Product id {
                    product.id} has no stock information")
    ...
    # optional: set shipping/billing ids if your Orders model has these columns (you added shipping relation)
...
def create_order_buy_now(
    ...
    if quantity > product.stock_qty:
        raise ValueError(
            f"Not enough stock for product id {
                product.id}. Requested {quantity}, available {
                product.stock_qty}")
